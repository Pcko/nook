...
export function BuilderProvider({ editorRef, initialPage, children }) {
    const [page, setPage] = useState(initialPage);
    const [selectedElementId, setSelectedElementId] = useState(null);

    // Track selected element in GrapesJS
    useEffect(() => {
        const editor = editorRef.current;
        if (!editor) return;

        const onSelect = (cmp) => setSelectedElementId(cmp?.getId?.() || null);

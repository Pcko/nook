            ...
            <ul className="mt-4 max-h-64 overflow-y-auto bg-[#2D2E30] rounded-md p-3 shadow-inner">
                {hierarchyList.length ? (
                    hierarchyList.map((chain, index) => (
                        <li key={index} className="text-sm py-2 px-3 bg-[#3A3B3D] rounded-md hover:bg-gray-600 transition">
                            <div className="font-semibold">{chain.header}</div>
                            <ul className="ml-4">
                                {chain.children.map((node) => (
                                    <li key={node.id} className="text-sm py-2 px-3 bg-[#4A4B4D] rounded-md hover:bg-gray-500 transition">
                                        {node.name}
                                    </li>
                                ))}
                            </ul>
